...
public class ServiceEnseignant implements IService<enseignant> {
    ...
    public List<enseignant> afficher1() throws SQLException {
        List<enseignant> enseignants = new ArrayList<>();
        String req = "SELECT * FROM enseignent ORDER BY ageE";
        Statement ste = connection.createStatement();
        ResultSet res = ste.executeQuery(req);
        while (res.next()) {
            enseignant enseignant = new enseignant();
            enseignant.setIdE(res.getInt("idE"));
            enseignant.setNom(res.getString("nomE"));
            enseignant.setPrenom(res.getString("prenomE"));
            enseignant.setEmail(res.getString("email"));
            enseignant.setMatier(res.getString("matier"));
            enseignant.setComptences(res.getString("comptence"));
            enseignant.setAge(res.getInt("ageE"));
            enseignant.setLangue(res.getString("langue"));
            enseignants.add(enseignant); } res.close();
        return enseignants; }
